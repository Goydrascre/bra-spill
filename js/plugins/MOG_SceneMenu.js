...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_SceneMenu');
	...
	Moghunter.scMenu_HPMeterY = Number(Moghunter.parameters['HP Meter Y-Axis'] || 140);
	Moghunter.scMenu_MPMeterX = Number(Moghunter.parameters['MP Meter X-Axis'] || 17);
	Moghunter.scMenu_MPMeterY = Number(Moghunter.parameters['MP Meter Y-Axis'] || 158);	
	Moghunter.scMenu_HPNumberX = Number(Moghunter.parameters['HP Number X-Axis'] || 50);
	...
	Moghunter.scMenu_MPNumberX = Number(Moghunter.parameters['MP Number X-Axis'] || 50);
...
Scene_Menu.prototype.createMonogatari = function() {
	 ...
	 this.createCommands();
	 // Command name label disabled
	 // this.createCommandName();
	 // Play time display disabled
	 // this.createPlayTime();
	 this.createLocation();
};
//==============================
...
Scene_Menu.prototype.updateMagicCircle = function() {
	 ...
	 if (this._magicCircle.rotation > Math.PI * 2) {this._magicCircle.rotation -= Math.PI * 2};
};
...
Scene_Menu.prototype.createOverlay = function() {
	this._overlay = new Sprite(ImageManager.loadMenusMain("Overlay"));
	this._field.addChild(this._overlay);
};
...
Scene_Menu.prototype.createCommands = function() {
	 ...
	 for (var i = 0; i < totalItems; i++) {
		 ...
		 this._compos[i] = [10, startY + (spacing * i)];
		 this._comzoom[i] = 0;
...
Scene_Menu.prototype.updateCommands = function() {
     for (var i = 0; i < this._commands.length; i++) {
		  if (this.isComEnabled(i)) {
		       var nx = this._compos[i][0] + 20;
			   var ny = this._compos[i][1];
			   if (this._commands[i].opacity < 255) {this._commands[i].opacity += 20};
			   if (this._commands[i].scale.x < 1.15) {this._commands[i].scale.x += 0.05};
		  } else {
		       var nx = this._compos[i][0];
			   var ny = this._compos[i][1];
			   if (this._commands[i].opacity < 255) {this._commands[i].opacity += 10};
			   if (this._commands[i].scale.x > 1.00) {this._commands[i].scale.x -= 0.05};
			   this._comzoom[i] = 0;
		  };
...
Scene_Menu.prototype.updateSelection = function() {
	if (this._statusWindow.active) {
		if (this._selField.opacity < 255) {this._selField.opacity += 15};
		...
		if (this._selField.opacity > 0) {this._selField.opacity -= 15};
	}
	for (var i = 0; i < this._selection.length; i++) {
		 ...
		 if (i === this._statusWindow._index) {
			 if (this._selection[i].opacity < 255) {this._selection[i].opacity += 15};
...
Scene_Menu.prototype.update = function() {
	...
	this.updateActorFlash();
};

//==============================
// * update Actor Flash
//==============================
Scene_Menu.prototype.updateActorFlash = function() {
	if (!this._characters) {return};
	if (!this._flashActive) {return};
	var idx = this._statusWindow._index;
	for (var i = 0; i < this._characters.length; i++) {
		var chr = this._characters[i]._char;
		if (i === idx) {
			this._flashTimer = (this._flashTimer || 0) + 1;
			var alpha = Math.floor(Math.abs(Math.sin(this._flashTimer * 0.04)) * 100);
			chr.setBlendColor([255, 255, 255, alpha]);
		} else {
			chr.setBlendColor([0, 0, 0, 0]);
		}
	}
};

//==============================
// * command Personal (Skills / Equip)
//==============================
var _mog_scmenu_commandPersonal = Scene_Menu.prototype.commandPersonal;
Scene_Menu.prototype.commandPersonal = function() {
	this._flashActive = true;
	this._flashTimer = 0;
	if (this._selField) {this._selField.visible = false};
	_mog_scmenu_commandPersonal.call(this);
};

//==============================
// * on Personal Ok
//==============================
var _mog_scmenu_onPersonalOk = Scene_Menu.prototype.onPersonalOk;
Scene_Menu.prototype.onPersonalOk = function() {
	this._flashActive = false;
	if (this._characters) {
		for (var i = 0; i < this._characters.length; i++) {
			this._characters[i]._char.setBlendColor([0, 0, 0, 0]);
		}
	}
	if (this._selField) {this._selField.visible = true};
	_mog_scmenu_onPersonalOk.call(this);
};

//==============================
// * on Personal Cancel
//==============================
var _mog_scmenu_onPersonalCancel = Scene_Menu.prototype.onPersonalCancel;
Scene_Menu.prototype.onPersonalCancel = function() {
	...
	if (this._selField) {this._selField.visible = true};
	_mog_scmenu_onPersonalCancel.call(this);
};
...
MBustMenu.prototype.posX = function() {
	...
	var tightSpace = Math.floor(space * 0.85);
    var offset = this._maxMembers > 1 ? 20 : 0;
    return Math.floor(Graphics.boxWidth / 2) - Math.floor(tightSpace * (this._maxMembers - 1) / 2) + (tightSpace * this._index) + offset;
};
...
MBustMenu.prototype.update = function() {
	...
	if (this._char.opacity < 255) {this._char.opacity += 10};
...
MCharStatus.prototype.createName = function() {
	...
	this._name.y = this._layout.y + Moghunter.scMenu_HPMeterY - 54;
...
MCharStatus.prototype.createHPMeter = function() {
	var mw = this._HPMeterImg.width / 3;
	var mh = this._HPMeterImg.height;
	var mx = this._layout.x + Moghunter.scMenu_HPMeterX;
	var my = this._layout.y + Moghunter.scMenu_HPMeterY;
	// black background
	this._hpMeterBg = new Sprite(new Bitmap(mw, mh));
	this._hpMeterBg.bitmap.fillRect(0, 0, mw, mh, '#000000');
	this._hpMeterBg.x = mx;
	this._hpMeterBg.y = my;
	this.addChild(this._hpMeterBg);
	// coloured bar
    this._hpMeter = new Sprite(this._HPMeterImg);
	this._hpMeterData = [mw, mh, -1, Math.randomInt(mw * 2)];
	this._hpMeter.x = mx;
	this._hpMeter.y = my;
	this.addChild(this._hpMeter);
	// outline on top
	this._hpMeterOutline = new Sprite(new Bitmap(mw + 2, mh + 2));
	this._hpMeterOutline.bitmap.fillRect(0, 0, mw + 2, mh + 2, '#4e0000');
	this._hpMeterOutline.bitmap.clearRect(1, 1, mw, mh);
	this._hpMeterOutline.x = mx - 1;
	this._hpMeterOutline.y = my - 1;
	this.addChild(this._hpMeterOutline);
	this.redrawHPMeter();
};
...
// * redraw HP Meter
//==============================
MCharStatus.prototype.redrawHPMeter = function() {
	var cw = Math.floor(this._hpMeterData[0] * this._actor.hp / Math.max(1, this._actor.mhp));
	this._hpMeter.setFrame(this._hpMeterData[3], 0, cw, this._hpMeterData[1]);
	this._hpMeterData[2] = this._actor.hp;
};
...
MCharStatus.prototype.createMPMeter = function() {
	var mw = this._MPMeterImg.width / 3;
	var mh = this._MPMeterImg.height;
	var mx = this._layout.x + Moghunter.scMenu_MPMeterX;
	var my = this._layout.y + Moghunter.scMenu_MPMeterY;
	// black background
	this._mpMeterBg = new Sprite(new Bitmap(mw, mh));
	this._mpMeterBg.bitmap.fillRect(0, 0, mw, mh, '#000000');
	this._mpMeterBg.x = mx;
	this._mpMeterBg.y = my;
	this.addChild(this._mpMeterBg);
	// coloured bar
    this._mpMeter = new Sprite(this._MPMeterImg);
	this._mpMeterData = [mw, mh, -1, Math.randomInt(mw * 2)];
	this._mpMeter.x = mx;
	this._mpMeter.y = my;
	this.addChild(this._mpMeter);
	// outline on top
	this._mpMeterOutline = new Sprite(new Bitmap(mw + 2, mh + 2));
	this._mpMeterOutline.bitmap.fillRect(0, 0, mw + 2, mh + 2, '#00195e');
	this._mpMeterOutline.bitmap.clearRect(1, 1, mw, mh);
	this._mpMeterOutline.x = mx - 1;
	this._mpMeterOutline.y = my - 1;
	this.addChild(this._mpMeterOutline);
	this.redrawMPMeter();
};
...
// * redraw MP Meter
//==============================
MCharStatus.prototype.redrawMPMeter = function() {
	var cw = Math.floor(this._mpMeterData[0] * this._actor.mp / Math.max(1, this._actor.mmp));
	this._mpMeter.setFrame(this._mpMeterData[3], 0, cw, this._mpMeterData[1]);
	this._mpMeterData[2] = this._actor.mp;
};
...
MCharStatus.prototype.createHPNumber = function() {
	...
	this._hpText = new Sprite(new Bitmap(160, 32));
	this._hpText.x = x;
	this._hpText.y = y;
	this._hpText.bitmap.fontSize = 18;
	this._hpLastValue = -1;
...
// * refresh HP Text
//==============================
MCharStatus.prototype.refreshHPText = function() {
	this._hpText.bitmap.clear();
	var mid = 60;
	this._hpText.bitmap.drawText(this._actor.hp + '/', 0, 0, mid, 32, 'right');
	this._hpText.bitmap.drawText(this._actor.mhp, mid, 0, 80, 32, 'left');
	this._hpLastValue = this._actor.hp;
};

//==============================
// * create HP Number2 (disabled - merged into HP Number)
//==============================
MCharStatus.prototype.createHPNumber2 = function() {
	// merged into createHPNumber as hp/maxhp
};
...
MCharStatus.prototype.createMPNumber = function() {
	...
	this._mpText = new Sprite(new Bitmap(160, 32));
	this._mpText.x = x;
	this._mpText.y = y;
	this._mpText.bitmap.fontSize = 18;
	this._mpLastValue = -1;
...
// * refresh MP Text
//==============================
MCharStatus.prototype.refreshMPText = function() {
	this._mpText.bitmap.clear();
	var mid = 60;
	this._mpText.bitmap.drawText(this._actor.mp + '/', 0, 0, mid, 32, 'right');
	this._mpText.bitmap.drawText(this._actor.mmp, mid, 0, 80, 32, 'left');
	this._mpLastValue = this._actor.mp;
};

//==============================
// * create MP Number2 (disabled - merged into MP Number)
//==============================
MCharStatus.prototype.createMPNumber2 = function() {
	// merged into createMPNumber as mp/maxmp
};
...
MCharStatus.prototype.createLayoutStatus = function() {
	 ...
	 this._layout.y = Graphics.boxHeight - 155 + Moghunter.scMenu_layoutStatusY;
...
MCharStatus.prototype.update = function() {
	...
	if (this._hpMeter && this._hpMeterData[2] !== this._actor.hp) {this.redrawHPMeter()};
	if (this._mpMeter && this._mpMeterData[2] !== this._actor.mp) {this.redrawMPMeter()};
	if (this._hpMeter) {
		this._hpMeterData[3] += 4;
		if (this._hpMeterData[3] > this._hpMeterData[0] * 2) {this._hpMeterData[3] = 0};
		this.redrawHPMeter();
	};
	if (this._mpMeter) {
		this._mpMeterData[3] += 4;
		if (this._mpMeterData[3] > this._mpMeterData[0] * 2) {this._mpMeterData[3] = 0};
		this.redrawMPMeter();
	};
	if (this._hpText && this._hpLastValue !== this._actor.hp) {this.refreshHPText()};
	if (this._mpText && this._mpLastValue !== this._actor.mp) {this.refreshMPText()};
...
// * makeCommandList
//==============================
var _mog_scmenu_makeCommandList = Window_MenuCommand.prototype.makeCommandList;
Window_MenuCommand.prototype.makeCommandList = function() {
	_mog_scmenu_makeCommandList.call(this);
	var hidden = ['status', 'formation', 'save', 'load'];
	this._list = this._list.filter(function(cmd) {
		return hidden.indexOf(cmd.symbol) === -1;
	});
};
