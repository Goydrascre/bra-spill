 ...
 * @target MZ
 * @author Aerosys
 * @plugindesc [Version 1.8.1] [MV & MZ]
 * 
 * @help
 * 
 * ----------------------------------------------------------------------------
...
Imported.MK_UICustomizer = '1.8.1';
...
if (hasPlugin('SRPG_BattlePrepare_MZ')) {
    ...
    Window_MenuStatus.prototype.drawItemImage = function(index) {
        if ($gameSystem.isSRPGMode()) {
            const actor     = this.actor(index);
            const faceRect  = this.faceRect(index); // <-- changed

            this.changePaintOpacity(!actor.srpgTurnEnd() && !actor.isRestricted());
            this.drawActorFace(actor, faceRect.x, faceRect.y, faceRect.width, faceRect.height); // <-- changed
            this.changePaintOpacity(true);
        } else {
    ...
    Window_MenuStatus.prototype.drawItemImage = function(index) {
        if ($gameSystem.isSRPGMode() && $gameSystem.isBattlePhase() == 'battle_prepare') {
            ...
            if ($gameParty.inLockedActorList(actor.actorId())) {
                this.drawIcon(
                    lockIconIndex,
                    faceRect.x + faceRect.width - ImageManager.iconWidth, // <-- changed
                    faceRect.y + faceRect.height - ImageManager.iconHeight, // <-- changed
                );
            }
        } else {
