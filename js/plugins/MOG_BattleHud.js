...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_BattleHud');
    ...
    // HUD POSITION
	Moghunter.bhud_pos_x = Number(Moghunter.parameters['Hud X-Axis'] || 10);
	Moghunter.bhud_pos_y = Number(Moghunter.parameters['Hud Y-Axis'] || 510);
	Moghunter.bhud_space_x = Number(Moghunter.parameters['Hud Space X'] || 160);
	...
	Moghunter.bhud_slideY = Number(Moghunter.parameters['Hud Slide Y'] || 120);
	...
	// TURN POSITION
	Moghunter.bhud_turn_visible = String(Moghunter.parameters['Turn Visible'] || false);
...
Spriteset_Battle.prototype.createEnemies = function() {
	_mog_bhud_sprbat_createEnemies.call(this);
	this.createDustLayer();
};

Spriteset_Battle.prototype.createDustLayer = function() {
	var troop = $dataTroops[$gameTroop._troopId];
	var speed = 1;
	var image = 'Dust';
	if (troop && troop.name) {
		var matchSpeed = troop.name.match(/<dust_speed:[ ]*(-?\d+(?:\.\d+)?)>/i);
		if (matchSpeed) {speed = Number(matchSpeed[1])};
		var matchImage = troop.name.match(/<dust_image:[ ]*(\S+)>/i);
...
var _mog_bhud_sprbat_update = Spriteset_Battle.prototype.update;
Spriteset_Battle.prototype.update = function() {
	_mog_bhud_sprbat_update.call(this);
	this.updateDustLayer();
};

Spriteset_Battle.prototype.updateDustLayer = function() {
	if (!this._dustSprite1) {return};
	if (!this._dustSprite1.bitmap.isReady()) {return};
	var bw = this._dustSprite1.bitmap.width;
	if (!this._dustReady) {
		this._dustSprite1.x = 0;
		this._dustSprite2.x = bw;
		this._dustReady = true;
	};
	this._dustSprite1.x -= this._dustSpeed;
	this._dustSprite2.x -= this._dustSpeed;
	if (this._dustSprite1.x <= -bw) {this._dustSprite1.x = this._dustSprite2.x + bw};
	if (this._dustSprite2.x <= -bw) {this._dustSprite2.x = this._dustSprite1.x + bw};
};
...
Battle_Hud.prototype.update_sprites = function() {
	this.update_meter_bg(this._hp_meter_bg);
	this.update_meter_bg(this._mp_meter_bg);
	this.update_meter_bg(this._tp_meter_bg);	
    this.update_active();
	this.update_visible();
	this.update_turn();
	this.update_face();	
...
//==============================
Battle_Hud.prototype.create_meter_bg = function(img, x, y, rotation, borderColor) {
	var bg = new Sprite(new Bitmap(1, 1));
	bg._meterImg = img;
	bg._meterX = x;
	bg._meterY = y;
	bg._meterRotation = rotation;
	bg._borderColor = borderColor || 'rgba(255,255,255,0.7)';
	bg._ready = false;
	this.addChild(bg);
	return bg;
};
...
//==============================
// * Update Meter Background
//==============================
Battle_Hud.prototype.update_meter_bg = function(bg) {
	if (!bg) {return};
	if (bg._ready) {return};
	if (!bg._meterImg.isReady()) {return};
	...
	bmp.fillRect(0, 0, 1, ih + 2, bc);
	bmp.fillRect(iw + 1, 0, 1, ih + 2, bc);
	bg.bitmap = bmp;
	bg.x = bg._meterX - 1;
	bg.y = bg._meterY - 1;
	bg.rotation = bg._meterRotation;
	bg._ready = true;
};
...
Battle_Hud.prototype.update_turn = function() {
	...
	if (!this._turn_blink) {this._turn_blink = [0,0]};
	var cw = this._face.bitmap.width / 5;
	var ch = this._face.bitmap.height;
	if (this._active) {
		this._face.setFrame(0, 0, cw, ch);
	} else {
		this._face.setFrame(cw, 0, cw, ch);
	};
};	
...
Battle_Hud.prototype.create_face = function() {
	...
	this._face_data = [0,0,false,false,false,0];
	this._face.ph = 0;
	this._face.animation = [-1,0,0,0,0,0,0,0,0];
	this._face.breathEffect = false;
	this._face.scaleY = 0;
...
Battle_Hud.prototype.update_face = function() {
	if (!this._face) {return};
	if (!this._face.bitmap.isReady()) {return};
	if (!this._face._frameSet) {
		var cw = this._face.bitmap.width / 5;
		var ch = this._face.bitmap.height;
		this._face.setFrame(cw, 0, cw, ch);
		this._face._frameSet = true;
	};
	this._face.x = this._pos_x + Moghunter.bhud_face_pos_x;
...
Battle_Hud.prototype.create_hp_meter = function() {
	...
	this.removeChild(this._hp_meter_bg);
	if (!this._battler) {return};
	var _hpx = this._pos_x + Moghunter.bhud_hp_meter_pos_x;
	var _hpy = this._pos_y + Moghunter.bhud_hp_meter_pos_y;
	var _hprot = Moghunter.bhud_hp_meter_rotation * Math.PI / 180;
	this._hp_meter_bg = this.create_meter_bg(this._hp_meter_img, _hpx, _hpy, _hprot, 'rgba(139,0,0,1)');
	this._hp_meter_red = new Sprite(this._hp_meter_img);
	this._hp_meter_red.x = _hpx;
	this._hp_meter_red.y = _hpy;
	this._hp_meter_red.rotation = _hprot;
...
Battle_Hud.prototype.create_mp_meter = function() {
	...
	this.removeChild(this._mp_meter_bg);
	if (!this._battler) {return};
	var _mpx = this._pos_x + Moghunter.bhud_mp_meter_pos_x;
	var _mpy = this._pos_y + Moghunter.bhud_mp_meter_pos_y;
	var _mprot = Moghunter.bhud_mp_meter_rotation * Math.PI / 180;
	this._mp_meter_bg = this.create_meter_bg(this._mp_meter_img, _mpx, _mpy, _mprot, 'rgba(0,0,139,1)');
	this._mp_meter_red = new Sprite(this._mp_meter_img);
	this._mp_meter_red.x = _mpx;
	this._mp_meter_red.y = _mpy;
	this._mp_meter_red.rotation = _mprot;
...
Battle_Hud.prototype.create_tp_meter = function() {
	...
	this.removeChild(this._tp_meter_bg);
	if (!this._battler) {return};
	var _tpx = this._pos_x + Moghunter.bhud_tp_meter_pos_x;
	var _tpy = this._pos_y + Moghunter.bhud_tp_meter_pos_y;
	var _tprot = Moghunter.bhud_tp_meter_rotation * Math.PI / 180;
	this._tp_meter_bg = this.create_meter_bg(this._tp_meter_img, _tpx, _tpy, _tprot, 'rgba(0,100,0,1)');
	this._tp_meter_red = new Sprite(this._tp_meter_img);
	this._tp_meter_red.x = _tpx;
	this._tp_meter_red.y = _tpy;
	this._tp_meter_red.rotation = _tprot;
